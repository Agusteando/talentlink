...
export default function PlantelList({ initialData }) {
  ...
  const handleDelete = async (id) => {
    if (
      !confirm(
        "¿Estás seguro de eliminar este plantel? Esta acción no se puede deshacer."
      )
    )
      return;
              ...
              <tr key={p.id} className="group hover:bg-slate-50/50 transition">
                ...
                <td className="px-6 py-4 text-center">
                  <div className="flex flex-col text-xs text-slate-500">
                    <span>
                      <b>{p._count.jobs}</b> Vacantes
                    </span>
                    <span>
                      <b>{p._count.users}</b> Usuarios
                    </span>
                  </div>
                </td>
        ...
        <form action={handleSubmit} className="p-6 space-y-5">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="col-span-2">
              ...
              <input
                name="name"
                defaultValue={plantel?.name}
                placeholder="Ej. Campus Ecatepec"
                required
                className="w-full border border-slate-300 rounded-lg p-2.5 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition"
              />
            </div>
            <div>
              <label className="block text-xs font-bold text-slate-500 mb-1 uppercase">
              ...
              <input
                name="code"
                defaultValue={plantel?.code}
                placeholder="ABC"
                maxLength={5}
                required
                className="w-full border border-slate-300 rounded-lg p-2.5 text-sm uppercase font-mono outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition"
              />
            </div>
          </div>
          ...
          <div>
            ...
            <textarea
              name="address"
              defaultValue={plantel?.address}
              rows={2}
              placeholder="Calle, Número, Colonia, Ciudad..."
              required
              className="w-full border border-slate-300 rounded-lg p-2.5 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition resize-none"
            />
          </div>
          ...
          <div className="flex items-center gap-3 bg-slate-50 p-3 rounded-lg border border-slate-100">
            <input
              ...
              name="isActive"
              value="true"
              defaultChecked={isEdit ? plantel.isActive : true}
              className="w-4 h-4 text-blue-600 rounded focus:ring-blue-500"
            />
            <label className="text-sm font-medium text-slate-700">
              Habilitar Plantel (Visible en Vacantes)
            </label>
          </div>
          ...
          <div className="pt-2 flex gap-3">
            <button
              type="button"
              onClick={close}
              className="flex-1 py-3 rounded-xl text-slate-600 font-bold hover:bg-slate-100 transition text-sm"
            >
              Cancelar
            </button>
            <button
              type="submit"
              className="flex-1 bg-slate-900 text-white py-3 rounded-xl font-bold hover:bg-blue-600 transition shadow-lg text-sm flex justify-center items-center gap-2"
            >
              <Building2 size={16} />
