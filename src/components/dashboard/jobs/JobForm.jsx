...
import { Save, Loader2, ArrowLeft, Search, Check, X, Briefcase, ChevronDown, Plus } from "lucide-react";
import { toast } from "react-hot-toast";
import Link from "next/link";

export default function JobForm({ initialData, plantels, jobTitles = [], isEdit = false, canManageCatalog = false }) {
  ...
  const handleCreatePuesto = async () => {
    ...
    try {
      ...
      if (res?.success && res?.puesto) {
        // Update local catalog and preselect
        ...
        <form action={handleSubmit} className="space-y-6">
          ...
          <div className="flex flex-col gap-2">
            <div className="flex items-center justify-between">
              <label className="block text-xs font-bold text-slate-500 uppercase">Puesto (Obligatorio)</label>
                  ...
                  {filteredPuestos.length === 0 ? (
                    <div className="p-4 text-center">
                      ...
                      {canManageCatalog ? (
                        ...
                        <span className="text-xs text-slate-400">Contacta al administrador para agregar uno.</span>
                      )}
                    </div>
                  ) : (
                    <ul className="py-2">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
          <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 flex flex-col">
            <div className="p-4 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
              <h3 className="font-bold text-slate-800">Registrar Nuevo Puesto</h3>
              <button onClick={() => setIsPuestoModalOpen(false)} className="p-1 rounded-lg hover:bg-slate-200 text-slate-500">
                <X size={18} />
              </button>
            </div>
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Nombre del Puesto</label>
                <input
                  value={newPuestoName}
            ...
            <div className="p-4 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
              ...
              >
                {creatingPuesto ? <Loader2 className="animate-spin" size={16} /> : <Plus size={16} />}
                Crear Puesto
              </button>
            </div>
          </div>
