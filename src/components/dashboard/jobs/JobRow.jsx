...
export default function JobRow({ job, isAdmin }) {
  ...
  const handleDelete = async () => {
    if (
      !confirm(
        "¿Estás seguro de eliminar esta vacante? Se borrará permanentemente."
      )
    )
      return;
  ...
  const isExpired =
    job.status === "OPEN" &&
    job.closingDate &&
    new Date() > new Date(job.closingDate);

  return (
    <div
      className={`bg-white rounded-xl border shadow-sm overflow-hidden group hover:shadow-md transition-all flex flex-col h-full ${
        isExpired
      ...
      <div className="p-6 flex-1">
        ...
        {job.salaryRange && (
          <p className="text-xs font-semibold text-emerald-700 mb-3">
            Rango salarial: {job.salaryRange}
          </p>
        )}

        <div
          className={`flex items-center gap-4 text-xs p-3 rounded-lg ${
            isExpired
