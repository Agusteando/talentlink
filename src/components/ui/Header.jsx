  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-white/10 bg-white/80 backdrop-blur-md shadow-sm">
      <div className="container mx-auto flex h-16 items-center justify-between px-4">
        {/* BRAND - Always links to Home / Public Site */}
            ...
            <div className="flex items-center gap-3 pl-3 border-l border-slate-200">
              {session.user.role !== 'CANDIDATE' && (
                <>
                    ...
                    {/* NEW: Dedicated Vacancy Management Tab */}
                    <Link href="/dashboard/jobs" className="flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-bold text-slate-600 hover:bg-slate-100 transition-all">
                        <Briefcase size={14} /> Mis Vacantes
                    </Link>
                </>
              )}
