...
export default async function SettingsHub() {
  ...
  const settingsOptions = [
    ...
    },
    {
      ...
      desc: "Elige cómo recibir alertas por plantel y puesto (panel, correo y push).",
      icon: Bell,
      href: "/dashboard/settings/notifications",
      color: "bg-indigo-50 text-indigo-600",
      visible: true, // cualquier usuario con acceso al dashboard puede ajustar sus notificaciones
    },
  ];
    ...
    <div className="min-h-screen bg-slate-50 p-8">
      <div className="max-w-5xl mx-auto">
        ...
        <p className="text-slate-500 mb-8">
          Panel central de administración de catálogos y reglas de negocio.
        </p>
              ...
              <Link key={opt.href} href={opt.href} className="group">
                <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:shadow-lg hover:border-blue-200 transition-all h-full flex flex-col">
                  <div className="flex items-center justify-between mb-4">
                    <div
                      className={`h-12 w-12 rounded-xl flex items-center justify-center ${opt.color}`}
                    >
                      <opt.icon size={24} />
                    </div>
                    <ArrowRight
                  ...
                  <h3 className="text-lg font-bold text-slate-800 mb-2 group-hover:text-blue-700 transition">
                    {opt.title}
                  </h3>
