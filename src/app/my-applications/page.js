// --- src\app\my-applications\page.js ---
        ...
        {myApps.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="text-slate-500 mt-2">
              Busca una vacante en la página principal.
            </p>
          </div>
        ) : (
          <div className="grid gap-6">
            {myApps.map((app) => (
              <div
                key={app.id}
                className="flex flex-col md:flex-row justify-between items-start md:items-center rounded-xl border border-gray-100 bg-white p-6 shadow-lg transition hover:shadow-xl"
              >
                <div>
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-2 text-sm text-slate-500">
                    <span className="rounded bg-slate-100 px-2 py-0.5 font-medium">
                      {app.job.plantel?.name || "Sin plantel"}
                    </span>
                    <span>•</span>
                    <span>
                      Aplicado el{" "}
                ...
                <div
                  className={`mt-4 md:mt-0 flex items-center gap-3 rounded-full border px-4 py-2 text-sm font-bold ${getStatusColor(
                    app.status
                  )}`}
                >
